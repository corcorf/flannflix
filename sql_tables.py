...
def get_lookup_dict(session, table_class, key_column, value_column):
    ...
    result = session.query(getattr(table_class, key_column), getattr(table_class, value_column)).all()
    return dict(result)

def check_full_search_operator(text_search_op):
    """
    check that text_search_op is a valid operator for a postgres full-test search query
    if not, break
    """
    if isinstance(text_search_op, str) and text_search_op.strip() in ['<->', '&', '|']:
        pass
    else:
...
def sanitize_search_term(search_string, regex_term=r"[^a-zA-Z\d\s]"):
    """
    remove characters from the search string that are likely to break the search
    """
...
def full_text_search(session, table, id_column, token_column, search_string, text_search_op="<->"):
    """
    ...
    search_string = add_sql_full_search_operators(search_string, text_search_op)
...
def full_text_search_movies(session, search_string, text_search_op="<->"):
    ...
    text_search_op is the operator that should be used in the postgres full text search
